...
class AnimalDataset(data.dataset.Dataset):
  def __init__(self, classes_file, transform):
    predicate_binary_mat = np.array(np.genfromtxt('c:/users/armin/desktop/Animals_with_Attributes2/predicate-matrix-binary.txt', dtype='int'))
    self.predicate_binary_mat = predicate_binary_mat
    self.transform = transform
    class_to_index = dict()

    with open('c:/users/armin/desktop/Animals_with_Attributes2/classes.txt') as f:
      index = 0
      for line in f:
    ...
    img_names = []
    img_index = []
    with open('c:/users/armin/desktop/{}'.format(classes_file)) as f:
      for line in f:
        class_name = line.strip()
        FOLDER_DIR = os.path.join('c:/users/armin/desktop/JPEGImages', class_name)
